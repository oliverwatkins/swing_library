...
public class WrapLayout extends FlowLayout {
	...
	private static final long serialVersionUID = -5268694863016167996L;
	 ...
	 * argument must be one of <code>WrapLayout</code>, <code>WrapLayout</code>, or <code>WrapLayout</code>.
	 * 
	...
	public Dimension minimumLayoutSize(Container target) {
		...
		// return minimum;
		Dimension minimum = new Dimension(1, 1);
		for (Component c : target.getComponents()) {
			minimum.width = Math.max(c.getMinimumSize().width, minimum.width);
			minimum.height = Math.max(c.getMinimumSize().height, minimum.height);
		}
		return minimum;
	}
