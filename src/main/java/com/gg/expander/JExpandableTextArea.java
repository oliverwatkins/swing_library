...
public class JExpandableTextArea extends JTextArea implements MouseMotionListener, MouseListener, KeyListener {
	 ...
	 * /| / | ----
	 * 
	 * This triangle represents the triangle for the bottom right corner of the viewport.
	 * 
	 * @return the triangle
	 */
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (mousePressedInTriangle) { // mouse was pressed in triangle so we can resize
			...
			newSize = new Dimension(Math.max(sizeAtPress.width - xDiff, 0), Math.max(sizeAtPress.height - yDiff, 0));
